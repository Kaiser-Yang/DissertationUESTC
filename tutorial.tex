...
\begin{document}
	...
	在上述过程中，分布式机器学习面临多方面挑战。首先，在\textbf{数据划分与资源异构}方面，不同的数据划分方式（例如IID与Non-IID划分）会影响梯度估计偏差与收敛速度；同时，节点算力、负载与可用资源可能随时间动态变化，若静态地确定任务分配与批大小，容易导致负载不均衡与资源利用率下降。其次，在\textbf{同步机制与收敛效率}方面，需要在通信频率与优化收敛之间权衡：过于频繁的同步会引入显著通信开销与等待时间，而过长的本地更新间隔可能导致梯度陈旧或收敛震荡，从而增加达到目标精度所需的迭代轮数。再次，在\textbf{通信效率与稳定性}方面，带宽受限、时延与网络抖动会显著放大训练的端到端耗时；因此常需结合量化、稀疏化、裁剪Top-$k$与误差反馈等通信压缩技术，但压缩带来的额外计算开销、信息损失与收敛稳定性之间需要精细权衡。最后，在\textbf{慢节点与系统鲁棒性}方面，同步训练往往受到慢节点和故障节点的影响，容易形成尾部时延放大与同步屏障等待，进而拖慢整体训练；因此需要配合部分参与、异步更新、备份任务以及容错恢复机制来提升系统可用性。
	...
	由于FedAvg允许工作节点进行多步本地训练后再同步，相较于FedSGD其产生大量网络流量的周期更长，从而在带宽受限或高时延网络环境中更具实际可用性。然而，在工作节点数据分布为Non-IID时，过多的本地更新可能引入漂移现象，导致收敛速度变慢或训练不稳定。因此，如何根据网络状态与节点异构性自适应地选择本地训练步数、参与工作节点集合与同步频率，以在通信开销与收敛性能之间取得更优权衡，是当前联邦学习研究与系统优化中关键研究方向之一。
